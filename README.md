...
| قابلیت | توضیح |
|--------|--------|
| **برنامهٔ PRO (`manga_app.py`)** | یک فایل، سه حالت: **App** (پنجرهٔ دسکتاپ تیره با Tkinter)، **Web** (رابط Gradio تیره — خودکار روی Colab/Codespace بالا می‌آید و لینک عمومی می‌دهد)، **CLI تعاملی** (منوی فارسی می‌پرسد). لانچرهای `Manga.bat` و `manga.sh` منودار |
...
فونت‌های لحن (اختیاری): `--font-normal` (کودک) · `--font-shout` (خشم) · `--font-comedy-shout` (کمدی) · `--font-whisper` (زمزمه) · `--font-thought` (تفکر) · `--font-system` (سیستم/تگ) · `--font-letter` (نامه) · `--font-narrator` (راوی) · `--font-free` (متن آزاد) و … — برنامهٔ PRO این‌ها را بار اول خودکار دانلود و تنظیم می‌کند و پوشش فارسی هر کدام را تست می‌کند.
